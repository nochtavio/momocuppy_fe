...
class voucher extends CI_Controller {
  ...
  function show_object() {
    ...
    if ($checkadmin > 0) {
      ...
      $totalrow = $this->model_voucher->get_object(0, $voucher_name, $voucher_code, $active, $order)->num_rows();
      ...
      if ($totalrow > 0) {
        $query = $this->model_voucher->get_object(0, $voucher_name, $voucher_code, $active, $order, $limit, $size)->result();
        $temp = 0;
        foreach ($query as $row) {
          ...
          $data['expired_date'][$temp] = $row->expired_date != null ? date_format(date_create($row->expired_date), 'd F Y') : 'Not Set';
  ...
  function add_object() {
    ...
    if ($checkadmin > 0) {
      ...
      $data['result'] = "s";
      $this->model_voucher->add_object($voucher_name, $voucher_code, $discount, $expired_date);
  ...
  function edit_object() {
    ...
    if ($checkadmin > 0) {
      ...
      $data['result'] = "s";
      $this->model_voucher->edit_object($id, $voucher_name, $voucher_code, $discount, $expired_date);
  ...
  function get_object() {
    ...
    if ($checkadmin > 0) {
      ...
      foreach ($query as $row) {
        ...
        $data['expired_date'] = $row->expired_date != null ? date_format(date_create($row->expired_date), 'Y-m-d') : null;
