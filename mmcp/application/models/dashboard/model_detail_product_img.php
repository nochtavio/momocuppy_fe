...
class model_detail_product_img extends CI_Model {
  ...
  function get_object($id = 0, $id_product = 0, $visible = -1, $order = 0, $limit = 0, $size = 0) {
    ...
    $this->db->join('ms_product mp', 'mp.id = dpi.id_product');
  ...
  function add_object($id_product, $img) {
    ...
    $this->db->insert('dt_product_img', $data);
  }
  ...
  function edit_object($id, $img) {
    ...
    $this->db->where('id', $id);
    $this->db->update('dt_product_img', $data);
  }
